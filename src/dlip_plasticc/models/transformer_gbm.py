...
-----
    ...
    # inject precomputed GP features if available:
    test_dataset.raw_features = precomputed_gp_features
...
class TransformerGBMClassifier(Classifier):
    ...
    gp_featurizer
        ...
        If ``dataset.raw_features`` is already populated when ``train`` or
        ``predict`` is called, the GP features are read from there directly
        (no recomputation).
    transformer_epochs
    ...
    def _parse_sequence_features(self, dataset):
        ...
        Uses dataset.raw_features if already populated (e.g. pre-extracted
        via dataset.extract_raw_features(seq_featurizer) before calling
        train/predict), otherwise falls back to on-the-fly extraction which
        requires dataset.objects to be loaded.
        """
        if dataset.raw_features is None:
            if dataset.objects is None:
                raise AvocadoException(
                    "dataset.raw_features is not populated and dataset.objects "
                    "is not loaded.  Either call "
                    "dataset.extract_raw_features(seq_featurizer) before "
                    "training, or load the dataset with metadata_only=False."
                )
            dataset.extract_raw_features(self.sequence_featurizer)

        features = self.sequence_featurizer.select_features(dataset.raw_features)
    ...
    def _get_gp_feature_frame(self, dataset) -> pd.DataFrame:
        """Return raw GP features as a DataFrame indexed by object_id."""
        if dataset.raw_features is None:
            if dataset.objects is None:
                raise AvocadoException(
                    "dataset.raw_features is missing and dataset.objects is not "
                    "loaded. Cannot compute GP features on the fly."
                )
            dataset.extract_raw_features(self.gp_featurizer)

        selected = self.gp_featurizer.select_features(dataset.raw_features)
    ...
    def _train_transformer(
        ...
        if show_progress:
            iterator = tqdm(iterator, desc="[Stage 1] Transformer epochs",
                            dynamic_ncols=True)
        ...
        for epoch in iterator:
            ...
            msg = ("Epoch %d  train=%.5f  val=%.5f  acc=%.4f  lr=%.2e"
    ...
    def _train_lgbm(
        ...
        if early_stopping:
            callbacks.append(lgb.early_stopping(early_stopping, verbose=show_progress))

        print("[Stage 2] Training LightGBM on %d features (%d transformer + %d GP)…"
        ...
        print("[Stage 2] Best LightGBM iteration: %d  val logloss: %.5f"
              % (booster.best_iteration, booster.best_score["valid_0"]["multi_logloss"]))
        return booster
    ...
    def train(
        ...
        ----------
        dataset
            Avocado dataset.  Must be loaded with ``metadata_only=False``
            (observations are required for the sequence branch).
        ...
        gp_df = self._get_gp_feature_frame(dataset)

        object_classes = dataset.metadata["class"].values
        class_names    = np.unique(object_classes)
        class_map      = {c: i for i, c in enumerate(class_names)}
        class_indices  = np.array([class_map[c] for c in object_classes],
                                   dtype=np.int64)
    ...
    def predict(self, dataset, show_progress: bool = True) -> pd.DataFrame:
        """Generate class probability predictions for a dataset.

        The dataset must be loaded with ``metadata_only=False`` so that
        ``dataset.objects`` is available for the sequence branch.
        If ``dataset.raw_features`` is already populated with precomputed GP
        features, they will be used directly (no recomputation).
        """
        self._ensure_torch()
        self._ensure_lgbm()
