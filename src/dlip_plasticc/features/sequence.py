...
class PlasticcSequenceFeaturizer(Featurizer):
    ...
    Output
    ------
    features : ndarray, shape (n_samples, seq_len, 5)
    mask     : ndarray, shape (n_samples, seq_len)
        ...
        if T >= self.seq_len:
            ...
            mask = np.concatenate(
                [
                    ...
                    np.zeros(self.seq_len - T, dtype=np.float32),
                ]
            )
    ...
    def select_features(self, raw_features):
        """Convert raw features into `(features, mask)`.

        Accepts either:
        - a dict with keys ``sequence`` and ``mask``
        - a pandas DataFrame with columns ``sequence`` and ``mask``
        """
        ...
        raise AvocadoException(
            "Unsupported raw_features type in PlasticcSequenceFeaturizer."
        )